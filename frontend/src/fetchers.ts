...
export const fetchEntityChildren = async (
	universeId: string,
	entityId?: string,
) => {
	try {
		...
		if (!response.ok) {
			throw new Error(
				"An error occurred while fetching entity's children",
			);
		}
		return response.json();
	} catch (error) {
...
export const fetchTextBlock = async (entityId: string) => {
	try {
		...
		if (!response.ok)
			throw new Error("Entity not found", await response.json());
		return response.json();
	} catch (error) {
...
export const fetchSignup = async (verified_payload: {
	email: string;
	password: string;
}) => {
	try {
...
export const fetchLogin = async (login_payload: {
	email: string;
	password: string;
}) => {
