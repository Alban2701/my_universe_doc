...
async def update_universe(
    universe_id: int, universe_patch: PartialUniverse
) -> Universe:
    """
    Update a universe
    """
    try:
        updated_universe = await universe_controller.update_universe(
...
async def get_first_entities(universe_id: int) -> List[Entity]:
    ...
    """
    try:
        entities = await universe_controller.get_firsts_entities_from_a_universe(
            universe_id
        )
        return entities
    except HTTPException:
