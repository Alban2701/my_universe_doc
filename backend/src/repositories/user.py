...
class UserRepository(BaseRepository):
    ...
    async def register(self, user: InputUser) -> PartialUser:
        ...
        print(model_user)
        rows = await self.db.execute(sql, model_user)
    ...
    async def get_user_with_session_token(self, token_value: str) -> Optional[UserToken]:
        """
    ...
    async def is_logged_in(self, user_id: int) -> bool:
        ...
        try:
            token = await self.session_token_repository.get_token_by_user_id(user_id=user_id)
            return bool(token)
        except Exception as e:
            raise e

    async def get_user_admin_rights(self, user_id: int, universe_id: int) -> UserUniverseRole | None:
        """
