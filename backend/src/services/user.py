...
class UserService:
    ...
    async def login(self, credentials: LoginUser) -> str:
        ...
        session = await self.session_token_repository.create_session_token(
            unoptional(user.id)
        )
    ...
    async def get_current_user(self, token_value: str) -> UserToken:
        ...
        """
        return unoptional(
            await self.user_repository.get_user_with_session_token(token_value)
        )
    ...
    async def is_super_admin(self, user_id: int, universe_id: int) -> bool:
        ...
        """
        user_role = unoptional(
            await self.user_repository.get_user_role(user_id, universe_id)
        )
        rights = user_role.admin_role
